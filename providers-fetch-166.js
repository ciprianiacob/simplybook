...
function loadHtml2Canvas() {
    ...
    script.onload = function() {
        console.log('html2canvas script has been loaded.');
        // Now you can use html2canvas in your code
        // Place the relevant code that uses html2canvas here
    };

    // Listen for any errors during script loading
    script.onerror = function() {
        console.error('Error loading html2canvas script.');
    };
}

// Call the function to load html2canvas
loadHtml2Canvas();
    ...
    if (iframe.length > 0) {
        ...
        if (calendarCanvas.length > 0) {
            try {
                ...
                const canvasDataUrl = await html2canvas(iframe[0]).then(canvas => canvas.toDataURL());

                // Set the canvas image source directly
                calendarCanvas[0].src = canvasDataUrl;
