																	...
																	if (providerProp.attr('data-provider-availability-first-day') === "null") {
																		try {
																			...
																			if (iframe.length > 0) {
																				...
																				takeScreenshotAndAppend(iframe[0], widgetIframeCanvas);
  ...
  async function takeScreenshotAndAppend(iframe, canvasContainer) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            context.drawImage(iframeWindow, 0, 0, width, height);
...
// Example usage:
const iframe = document.querySelector('.modal iframe.sb-widget-iframe');
const widgetIframeCanvas = document.querySelector('.sb-widget-iframe-canvas');

await takeScreenshotAndAppend(iframe, widgetIframeCanvas);
