(function() {
	...
	async function processData() {
		try {
			...
			$(document).ready(function() {
				...
				for (const service of filteredServices) {
                    ...
                    $(document).on('click', '.item__read-more', function() {
        const modalId = $(this).data('target');
        const iframe = $(`${modalId} iframe.sb-widget-iframe`);

        if (iframe.length > 0) {
            const locationValue = /* provide the location value */;
            const categoryId = /* provide the category ID */;
            const correspondingServiceId = /* provide the corresponding service ID */;
            const providerId = /* provide the provider ID */;

            const iframeSrc = `?widget-type=iframe#book/location/${locationValue}/category/${categoryId}/service/${correspondingServiceId}/count/1/addons/no/provider/${providerId}/`;
        ...
        const iframe = $(this).find('iframe.sb-widget-iframe');
