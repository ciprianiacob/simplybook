(function() {
    ...
    if (iframe.length > 0) {
        ...
        // Load iframe content onto the canvas
        const canvasContext = calendarCanvas.getContext('2d');
        const iframeImage = new Image();

        // Set the crossOrigin attribute to handle potential CORS issues
        iframeImage.crossOrigin = 'anonymous';

        iframeImage.onload = function () {
            // Draw the iframe content onto the canvas
            canvasContext.drawImage(iframeImage, 0, 0, calendarCanvas.width, calendarCanvas.height);
        };

        iframeImage.src = iframeSrc;
    }
																		} catch (error) {
