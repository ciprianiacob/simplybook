...
async function calculateDoHomeArtTimingStatistics() {
	try {
		...
		var calcDomContentLoadedTimeSeconds = (domContentLoadedTime + 5) / 1000;
		setTimeout(setBeforePseudoElementBackground, calcDomContentLoadedTimeSeconds * 1000);
		var loadTimeSeconds = (loadTime + 5) / 1000;
		domContentLoadedTimeSeconds = Number(((domContentLoadedTime + 5) / 1000).toFixed(2));
		finalDelayDoHomeArtValue = Number((calcDomContentLoadedTimeSeconds + loadTimeSeconds).toFixed(2));
		return {
			finalDelayDoHomeArtValue: finalDelayDoHomeArtValue,
			domContentLoadedTimeSeconds: domContentLoadedTimeSeconds
		};

		console.log(`DOMContentLoaded: ${calcDomContentLoadedTimeSeconds.toFixed(2)} s`);
		console.log(`Load: ${loadTimeSeconds.toFixed(2)} s`);
		console.log(`DELAY TIME IN SECONDS: ${(calcDomContentLoadedTimeSeconds + loadTimeSeconds).toFixed(2)} s`);
...
async function setDoHomeArtPreloaderStatus() {
	...
	const timingStats = await calculateDoHomeArtTimingStatistics();
	if (!timingStats || !timingStats.finalDelayDoHomeArtValue || !timingStats.domContentLoadedTimeSeconds) {
		console.error("Timing statistics are incomplete or invalid.");
		return;
	}
	finalDelayDoHomeArtValue = timingStats.finalDelayDoHomeArtValue;
	domContentLoadedTimeSeconds = timingStats.domContentLoadedTimeSeconds;
