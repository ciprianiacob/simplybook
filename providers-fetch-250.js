																	...
																	if (providerProp.attr('data-provider-availability-first-day') === "null") {
																		try {
																			...
																			if (iframe.length > 0) {
																				...
																				canvas.width = iframe.width(); // Set canvas width to iframe width
																				canvas.height = iframe.height(); // Set canvas height to iframe height

																				console.log('Canvas size:', canvas.width, 'x', canvas.height);

																				context.clearRect(0, 0, canvas.width, canvas.height);
																				context.fillStyle = 'white'; // Set a background color if needed
																				context.fillRect(0, 0, canvas.width, canvas.height);

																				console.log('Capturing screenshot...');

																				var dataURL = canvas.toDataURL('image/jpeg'); // Change to desired format

																				console.log('Data URL:', dataURL);

																				// Set the src attribute of the manually added img element
																				var manuallyAddedImg = widgetIframeCanvas.find('img');
																				if (manuallyAddedImg.length > 0) {
																					manuallyAddedImg.attr('src', dataURL);
																					console.log('Image src set.');
																				} else {
																					console.error('Manually added img element not found.');
																				}
