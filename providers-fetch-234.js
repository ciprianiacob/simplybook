(function() {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            console.log("Cloning iframe contents...");
            ...
            // Clone the iframe contents
            const clonedContents = iframeDocument.body.cloneNode(true);

            // Append the cloned contents to the container
            canvasContainer.empty().append(clonedContents);

            console.log("Cloned contents appended to the container.");

            resolve();
        } catch (error) {
            console.error("Error during cloning iframe contents:", error);
            reject(error);
        }
    });
}
	...
	loadExternalStylesheet('https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css');
  loadExternalStylesheet('https://cdn.jsdelivr.net/gh/zalog/placeholder-loading@develop/dist/css/placeholder-loading.min.css');
  loadExternalStylesheet('https://cdn.jsdelivr.net/gh/ciprianiacob/fancyboxdha@main/fancyboxdha-sb.css');
  processData();
