...
if (iframe.length > 0) {
    ...
    // Create a single canvas element for capturing content
    const calendarCanvas = document.createElement('canvas');
    calendarCanvas.width = 300; // Set the desired width
    calendarCanvas.height = 200; // Set the desired height
    calendarCanvas.setAttribute('class', 'calendar-canvas');

    // Append the canvas to the body (or any desired location)
    document.body.appendChild(calendarCanvas);

    console.log('Iframe and canvas created');

    // Set the iframe source
    captureIframe.src = iframeSrc;

    // Wait for the iframe to load
    captureIframe.onload = function () {
        console.log('Iframe loaded');

        // Clear the canvas before drawing new content
        const canvasContext = calendarCanvas.getContext('2d');
        canvasContext.clearRect(0, 0, calendarCanvas.width, calendarCanvas.height);

        // Draw the iframe content onto the calendarCanvas
        canvasContext.drawImage(captureIframe.contentDocument.documentElement, 0, 0, calendarCanvas.width, calendarCanvas.height);

        console.log('Iframe content drawn on canvas');
        ...
        console.log('Image element appended to body');
        ...
        document.body.removeChild(calendarCanvas);
