...
function startCountdown(initialValue) {
	let countdownValue = initialValue; // Valoarea de pornire a cronometrului

	// Funcția care se va apela la fiecare secundă pentru a decrementa valoarea cronometrului
	const updateCountdown = () => {
		if (countdownValue > 0) {
			countdownValue--; // Decrementăm valoarea cronometrului cu o secundă, dacă nu a ajuns la 0
		} else {
			clearInterval(countdownInterval); // Oprim cronometrul când ajunge la 0
		}
	};

	// Pornim cronometrul actualizându-l la fiecare secundă
	const countdownInterval = setInterval(updateCountdown, 1000);

	// Returnăm o funcție care va opri cronometrul și va returna valoarea finală când va fi apelată
	return () => {
		clearInterval(countdownInterval); // Oprim cronometrul
		return countdownValue; // Returnăm valoarea finală a cronometrului
	};
}
...
async function calculateDoHomeArtTimingStatistics() {
	try {
		...
		const countdown = startCountdown(finalDelayDoHomeArtValue);

		// Exemplu de folosire: Așteptăm 5 secunde și apoi afișăm valoarea cronometrului
		setTimeout(() => {
			const finalCountdownValue = countdown(); // Opim cronometrul și obținem valoarea finală
			console.log("Valoarea finală a cronometrului este:", finalCountdownValue, "secunde.");
		}, 0); // Așteptăm 5 secunde pentru a simula trecerea timpului
