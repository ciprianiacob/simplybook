...
function calculateTimingStatistics() {
    try {
        ...
        if (!timing || timing.domContentLoadedEventEnd <= 0 || timing.loadEventEnd <= 0 || timing.navigationStart <= 0) {
            console.warn("Timing data is incomplete or invalid.");
            if (!timing) {
                console.warn("performance.timing is not available.");
            } else {
                if (timing.domContentLoadedEventEnd <= 0) {
                    console.warn("domContentLoadedEventEnd is invalid or missing.");
                }
                if (timing.loadEventEnd <= 0) {
                    console.warn("loadEventEnd is invalid or missing.");
                }
                if (timing.navigationStart <= 0) {
                    console.warn("navigationStart is invalid or missing.");
                }
            }
            return;
        }
