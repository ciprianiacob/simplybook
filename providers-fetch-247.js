																	...
																	if (providerProp.attr('data-provider-availability-first-day') === "null") {
																		try {
																			...
																			if (iframe.length > 0) {
																				...
																				// Set the src attribute of the iframes
																				iframe.attr('src', iframeSrc);

																				console.log("Capturing screenshot for iframe:", iframeSrc);

																				// Find the correct iframe with the matching src attribute
																				const correctIframe = iframe.filter('[src="' + iframeSrc + '"]');

																				if (correctIframe.length > 0) {
																					console.log("Correct iframe found:", correctIframe[0]);

																					// Execute html2canvas for the correct iframe
																					html2canvas(correctIframe[0])
																						.then(function(canvas) {
																							console.log("html2canvas executed successfully.");

																							// Find the existing img element inside .sb-widget-iframe-canvas
																							var existingImg = widgetIframeCanvas.find('img');

																							if (existingImg.length > 0) {
																								console.log("Existing img element found:", existingImg[0]);

																								// Update the src attribute of the existing img element
																								existingImg.attr('src', canvas.toDataURL());
																								console.log('Image src updated for the correct iframe.');
																							} else {
																								console.error('Existing img element not found.');
																							}
																						})
																					...
																					console.error('Correct iframe not found.');
																				}
																			}
