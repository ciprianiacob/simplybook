...
fetch(jsonProviderUrl)
    ...
    .then(providerData => {
        console.log('Provider Data:', providerData); // Log provider data for debugging
        fetch(jsonServicesUrl)
            .then(response => response.json())
            .then(serviceData => {
                console.log('Service Data:', serviceData); // Log service data for debugging
                $(document).ready(function () {
                    $('#sb_booking_content .category-item').each(function () {
                        ...
                        console.log('Updating for Category ' + categoryId + ' and Provider ' + providerId); // Log for debugging
