...
async function initBookingSteps() {
	if (typeof finalDelayDoHomeArtValue === 'number' && !isNaN(finalDelayDoHomeArtValue) && appendedPreloader) {
		setTimeout(async function() {
			const stepContent = document.querySelector('.step-content');
			var setStepBackLink = document.querySelector('#steps-content #sb_back_button a');
			var appendedPreloaderForContent = document.querySelector('.preloader-is-not-active #appendedPreloader');
			var root = document.querySelector(":root");
			if (setStepBackLink) {
				setStepBackLink.style.pointerEvents = "none";
			}

			if (stepContent && setStepBackLink && appendedPreloaderForContent && root) {
				...
				switch (true) {
					case stepContent.classList.contains('location-step') && document.getElementById('sb_prerequisites_step_container'):
						...
						console.log('Case 0: step-content has class location-step with id "sb_prerequisites_step_container"');
						break;
					case stepContent.classList.contains('location-step') && !document.getElementById('sb_prerequisites_step_container'):
