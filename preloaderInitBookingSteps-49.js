...
function hideAppendedPreloader() {
	var appendedPreloader = document.getElementById('appendedPreloader');
	if (appendedPreloader) {
		appendedPreloader.style.display = 'none';
	}
}

function displayAppendedPreloader() {
	var appendedPreloader = document.getElementById('appendedPreloader');
	if (appendedPreloader) {
...
function hideAppendedPreloaderFromSteps() {
	var appendedPreloader = document.querySelector('.preloader-is-not-active:not(.content-is-loaded) #appendedPreloader');
	if (appendedPreloader) {
		appendedPreloader.style.display = 'none';
	}
}

function displayAppendedPreloaderFromSteps() {
	var appendedPreloader = document.querySelector('.preloader-is-not-active:not(.content-is-loaded) #appendedPreloader');
	if (appendedPreloader) {
		appendedPreloader.style.display = 'block';
	}
...
async function initBookingSteps() {
	if (typeof finalDelayDoHomeArtValue === 'number' && !isNaN(finalDelayDoHomeArtValue)) {
		setTimeout(async function() {
			...
			if (stepContent) {
				displayAppendedPreloaderFromSteps();
				switch (true) {
					case stepContent.classList.contains('location-step') && !document.getElementById('sb_prerequisites_step_container'):
						...
						hideAppendedPreloaderFromSteps();
...
function setBeforePseudoElementBackground() {
	document.querySelector(":root").style.setProperty("--main-content-bg", 'rgba(255,255,255,0)');
}
