																...
																if (correspondingService) {
																	...
																	if (providerProp.attr('data-provider-availability-first-day') === "null") {
																		try {
																			...
																			var widgetIframe = categoryItem.find('.modal iframe.sb-widget-iframe');
																			var widgetIframeCanvas = categoryItem.find('.sb-widget-iframe-canvas:first');
																			...
																			if (widgetIframe.length > 0) {
																				...
																				widgetIframe.attr('src', iframeSrc);

																				if (widgetIframeCanvas.length > 0) {
																					console.log("widgetIframeCanvas.length: ", widgetIframeCanvas.length);

																					widgetIframe.on('load', function() {
                                            console.log("widgetIframe was loaded");
																						widgetIframeCanvas.each(function() {
																							var calendarCanvas = $(this);
																							console.log('HTML content:', calendarCanvas.html());
                                              
																							if (calendarCanvas.attr('src')) {
																								var canvas = document.createElement('canvas');
																								canvas.width = calendarCanvas[0].offsetWidth;
																								console.log('canvas.width:', canvas.width);
																								canvas.height = calendarCanvas[0].offsetHeight;
																								console.log('canvas.height:', canvas.height);

																								var context = canvas.getContext('2d');

																								context.fillStyle = getComputedStyle(calendarCanvas[0]).backgroundColor;
																								context.fillRect(0, 0, canvas.width, canvas.height);
																								context.font = getComputedStyle(calendarCanvas[0]).font;
																								context.fillStyle = getComputedStyle(calendarCanvas[0]).color;
																								context.fillText(calendarCanvas.text(), 10, 20);
																								...
																								widgetIframeCanvas.append(individualWidgetIframeCanvas);
																					...
																					console.error("widgetIframeCanvas.length: ", widgetIframeCanvas.length);
																				}
																			} else {
																				console.error("No widgetIframe found");
																			}
