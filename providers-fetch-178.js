																...
																if (correspondingService) {
																	...
																	if (providerProp.attr('data-provider-availability-first-day') === "null") {
																		try {
																			...
																			const iframeCanvas = await html2canvasModule(iframe[0]);

																			// Check if the willReadFrequently attribute is supported
																			if ('willReadFrequently' in iframeCanvas) {
																				// Set the willReadFrequently attribute to true
																				iframeCanvas.willReadFrequently = true;
																				console.log('willReadFrequently attribute set to true.');
																			} else {
																				console.warn('willReadFrequently attribute is not supported.');
																			}

																			// ... (existing code)

																			// Capture the iframe content
																			const providerCanvas = await html2canvasModule(iframe[0]);

																			// Check if the willReadFrequently attribute is supported
																			if ('willReadFrequently' in providerCanvas) {
																				// Set the willReadFrequently attribute to true
																				providerCanvas.willReadFrequently = true;
																				console.log('willReadFrequently attribute set to true.');
																			} else {
																				console.warn('willReadFrequently attribute is not supported.');
																			}
