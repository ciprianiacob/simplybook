...
function setStepServiceBackLink() {
	...
	function handleHrefChange(mutationsList, observer) {
		for (let mutation of mutationsList) {
			if (mutation.type === 'attributes' && mutation.attributeName === 'href') {
				if (setStepBackLink.getAttribute('href') === '#book/count/1/') {
	...
	const observer = new MutationObserver(handleHrefChange);
	observer.observe(setStepBackLink, {
		attributes: true
	});
