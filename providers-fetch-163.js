(function() {
    ...
    if (calendarCanvas.length === 0) {
        ...
        const newCalendarCanvas = document.createElement('canvas');
        newCalendarCanvas.width = 300; // Set the desired width
        newCalendarCanvas.height = 200; // Set the desired height
        newCalendarCanvas.setAttribute('class', 'calendar-canvas');

        // Append the canvas to the canvas-calendar div
        canvasCalendar.append(newCalendarCanvas);

        // Set the calendarCanvas variable to the newly created canvas
        calendarCanvas = newCalendarCanvas;
    }
    ...
    if (iframe.length > 0) {
        ...
        // Use html2canvas to capture the iframe content and draw it onto the canvas
        html2canvas(iframe[0]).then(function (canvas) {
            try {
                const canvasContext = calendarCanvas.getContext('2d');

                // Draw the captured content onto the canvas
                canvasContext.drawImage(canvas, 0, 0, calendarCanvas.width, calendarCanvas.height);

                console.log('Canvas updated successfully');
            } catch (error) {
                console.error('Error drawing iframe content onto canvas:', error);
            }
