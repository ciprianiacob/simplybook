(function() {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Convert iframe content to Blob
            const iframeBlob = new Blob([new XMLSerializer().serializeToString(iframeDocument)], { type: 'image/svg+xml' });

            // Create an object URL for the Blob
            const iframeUrl = URL.createObjectURL(iframeBlob);

            // Ensure the iframe content is an HTMLImageElement
            const iframeImage = new Image();
            iframeImage.src = iframeUrl;

            // Wait for the image to load before drawing
            iframeImage.onload = function () {
                const context = canvas.getContext("2d");
                context.drawImage(iframeImage, 0, 0, width, height);
                ...
                canvasContainer.appendChild(screenshotImage);
            ...
            iframeImage.onerror = function (error) {
                console.error("Error loading iframe content as an image:", error);
                reject(error);
            };
        } catch (error) {
