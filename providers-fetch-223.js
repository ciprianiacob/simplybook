(function() {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Replace 'yourElementId' with the actual ID of the element you want to capture
      const node = iframeDocument.getElementById('yourElementId');

      if (!node) {
        console.error("Element with ID 'yourElementId' not found in the iframe");
        throw new Error("Element not found");
      }

      // Exclude font elements from the screenshot
      function filterFonts(element) {
        return element.tagName !== 'FONT';
      }

      // Capture the screenshot using dom-to-image library
      domtoimage.toPng(node, { filter: filterFonts })
        .then((dataURL) => {
